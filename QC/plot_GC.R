...
#*********************	GC_Content
out_GC=paste(sample_name,".GC.pdf",sep="")
rt1=read.table(paste(R1_file_prefix,".gc.stat",sep=""))
if(file.exists(paste(R2_file_prefix,".gc.stat",sep=""))) {
	rt2=read.table(paste(R2_file_prefix,".gc.stat",sep=""))
	rt=rt1+rt2
}else{
	rt=rt1
}
...
#*********************	MeanQV Plot
out_MeanQV=paste(sample_name,".MeanQual.pdf",sep="")
pdf(out_MeanQV,width=12,height=8)

par(mfrow=c(2,1))
rt = read.table(paste(R1_file_prefix,".qmean.stat",sep=""))
length=length(rt[[1]])
data=rt[,2]
pos =rt[,1]
names(data)=pos
...
if(file.exists(paste(R2_file_prefix,".qmean.stat",sep=""))) {
rt = read.table(paste(R2_file_prefix,".qmean.stat",sep=""))
length=length(rt[[1]])
data=rt[,2]
pos =rt[,1]
names(data)=pos
